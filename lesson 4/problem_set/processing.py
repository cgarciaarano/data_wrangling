...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            new_line = {}
            new_line['classification'] = {}
            for csv_field,field in FIELDS.iteritems():

                # strip leading and ending whitespace from all fields, if there is any
                value = line[csv_field].strip()
                ...
                # if 'name' is "NULL" or contains non-alphanumeric characters, set it to the same value as 'label'.
                if csv_field == 'name' and (not value or not re.match(pat_alphanum, value)):
                  value = new_line['label'] # Careful, it may not be populated yet

                # trim out redundant description in parenthesis from the 'rdf-schema#label' field, like "(spider)"
                if csv_field == 'rdf-schema#label' and value:
                  value = re.sub(pat_parenth, '', value).strip()
                
                # 'synonym' handling, stripping the "{}" characters and splitting the string on "|"
                if csv_field == 'synonym' and value:
                  value = value.strip('{}').split('|')

                if csv_field in ('family_label','class_label','phylum_label','order_label','kingdom_label','genus_label'):
                  new_line['classification'][field] = value
                else:
