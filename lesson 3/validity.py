...
def is_valid_year(field):
    if isinstance( field, ( int, long ) ):
        if field >= 1886 and  field <= 2014:
            print 'good'
            return True
    print('bad: {0}'.format(field))
    return False
            ...
            with open(output_bad, "w") as b:
                ...
                for row in reader:
                    if 'dbpedia.org' in row['URI']:
                        row['productionStartYear'] = convert(row['productionStartYear'])
