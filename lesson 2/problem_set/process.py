...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in table.find_all('tr', class_='dataTDRight'):
            row_dict = { "courier" : info["courier"],
                        "airport": info["airport"],
            }

            column = row.find('td')
            row_dict["year"] = int(column.text)
            column = column.find_next_sibling()
            if 'total' in column.text.lower():
                continue
            row_dict["month"] = int(column.text)
            column = column.find_next_sibling()
            domestic = int(column.text.replace(',',''))
            column = column.find_next_sibling()
            international = int(column.text.replace(',',''))
            row_dict["flights"] = {
                "domestic":domestic,
